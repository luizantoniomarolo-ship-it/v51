    ...
    def check_chrome_installation(self) -> bool:
        ...
        for path in chrome_paths:
            if '*' in path:
                ...
                if matching_paths:
                    # Filtra apenas os caminhos que existem e são executáveis
                    valid_paths = [p for p in matching_paths if os.path.exists(p) and os.access(p, os.X_OK)]
                    if valid_paths:
                        self.chrome_paths.extend(valid_paths)
                        chrome_available = True
                        logger.info(f"✅ Chrome encontrado no Nix (wildcard): {valid_paths}")
                        break
